...
@testset verbose = true "ParQuery" begin
    @testset "Case №1: Cut" begin
        ...
        for exp_obj in exp_objs
            ...
            @test Serde.Query.ParQuery.cut(exp_str, "=") == exp_obj
        end
    end

    @testset "Case №2: Empty string" begin
        @test Serde.Query.ParQuery.unescape("") == ""
    end
    ...
    @testset "Case №3: String without escaped characters" begin
        ...
        for exp_str in exp_strs
            @test Serde.Query.ParQuery.unescape(exp_str) == exp_str
        end
    end

    @testset "Case №4: Single escaped character" begin
        @test Serde.Query.ParQuery.unescape("%20") == " "
        @test Serde.Query.ParQuery.unescape("%3C") == "<"
        @test Serde.Query.ParQuery.unescape("%3E") == ">"
        @test Serde.Query.ParQuery.unescape("%26") == "&"
        @test Serde.Query.ParQuery.unescape("%23") == "#"
        @test Serde.Query.ParQuery.unescape("%25") == "%"
    end

    @testset "Case №5: Multiple escaped characters" begin
        @test Serde.Query.ParQuery.unescape("hello%20world") == "hello world"
        @test Serde.Query.ParQuery.unescape("%3C%3E%26%23%25") == "<>&#%"
    end

    @testset "Case №6: Invalid escaped characters" begin
        @test_throws Serde.Query.EscapeError Serde.Query.ParQuery.unescape("%")
        @test_throws Serde.Query.EscapeError Serde.Query.ParQuery.unescape("%0")
        @test_throws Serde.Query.EscapeError Serde.Query.ParQuery.unescape("%z2")
    end
    ...
    @testset "Case №7: Decode key" begin
        ...
        for (exp_str, exp_obj) in exp_objs
            @test Serde.Query.ParQuery.decode_key(exp_str) == exp_obj
        end

        @test_throws Serde.Query.QueryParsingError Serde.Query.ParQuery.validate_key(
            "invalid_key;",
        )
    end
    ...
    @testset "Case №8: Decode value" begin
        ...
        for (exp_str, exp_obj) in exp_objs
            @test Serde.Query.ParQuery.decode_value(exp_str) == exp_obj
        end
    end
    ...
    @testset "Case №9: Parse exp_str" begin
        ...
        @test Serde.Query.ParQuery.parse(exp_str) == exp_obj
    ...
    @testset "Case №10: Parse exp_str with different Dict type" begin
        ...
        exp_str = "name=John+Doe&age=25&city=New+York"
        @test Serde.Query.parse_query(exp_str, dict_type = IdDict) == exp_obj
    end

    @testset "Case №11: Parse exp_str with other string types" begin
        exp_obj = Dict("name" => "John Doe", "age" => "25", "city" => "New York")
        exp_str = "name=John+Doe&age=25&city=New+York"
        @test Serde.Query.parse_query(exp_str) == exp_obj
        @test Serde.Query.parse_query(Vector{UInt8}(exp_str)) == exp_obj
        @test Serde.Query.parse_query(SubString(exp_str, 1)) == exp_obj
    end

    @testset "Case №12: Parse exp_str with different delimiter and/or backbone" begin
        exp_obj = Dict("name" => "John Doe", "age" => "25", "city" => "New York")
        exp_str = "name=John+Doe;age=25;city=New+York"
        @test Serde.Query.parse_query(exp_str, delimiter = ";") == exp_obj
        ...
        exp_obj = Dict("a" => ["1", "2", "3"])
        exp_str = "a=[1,2,3]"
        @test Serde.Query.parse_query(exp_str; backbone = Foo_1) == exp_obj
    end
end
