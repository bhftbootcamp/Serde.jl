...
export to_json, to_pretty_json, JsonSerializer
...
(ser_ignore_field(::Type{T}, k::Val{x}, v::V)::Bool) where {T,x,V} = Serde.ser_ignore_field(T, k, v)
...
function Strategy.serialize(s::JsonSerializer, data; kw...)
    return s.pretty ? to_pretty_json(data; kw...) : to_json(data; kw...)
end

function Strategy.serialize(s::JsonSerializer, f::Function, data; kw...)
