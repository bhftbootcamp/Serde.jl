...
function Base.show(io::IO, e::ParamError)
    print(io, "ParamError: parameter '$(e.key)' was not passed or has the value 'nothing'")
end

struct WrongType <: Exception
    maintype::DataType
    key::Any
    value::Any
    from_type::Any
...
function Base.show(io::IO, e::WrongType)
    print(io, "WrongType: for '$(e.maintype)' value '$(e.value)' has wrong type '$(e.key)::$(e.from_type)', must be '$(e.key)::$(e.to_type)'")
end
