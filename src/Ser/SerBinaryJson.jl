module SerBinaryJson

export to_binaryjson
...
function to_binaryjson(data; kw...)::Vector{UInt8}
    return BinaryJson.serialize(normalize_value(data))
end

function to_binaryjson(f::Function, data; kw...)::Vector{UInt8}
    return BinaryJson.serialize(normalize_value(f, data))
end
